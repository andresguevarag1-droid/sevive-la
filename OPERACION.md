...
| `/api/cron/capitulos` | Lunes y martes 17:00 UTC (11:00 CR) | Trae los videos nuevos del canal de YouTube del show a /capitulos (feed público, sin API). Necesita `YOUTUBE_CHANNEL_ID` en Vercel: en [youtube.com](https://youtube.com) con la cuenta del canal → Configuración → Configuración avanzada → *ID de canal* (empieza con `UC`). Respaldo manual: Studio → 📺 Capítulos → pegar el enlace |
