...
| `/api/cron/capitulos` | Lunes y martes 17:00 UTC (11:00 CR) | Trae los videos nuevos del canal de YouTube del show a /capitulos (feed público, sin API). Necesita `YOUTUBE_CHANNEL_ID` en Vercel — sirve el **@handle tal cual** (ej. `@SeViveLatam`) o el ID `UC…`. Respaldo manual: Studio → 📺 Capítulos → pegar el enlace |
